...
final class ColorImpl implements Color {
    ...
    public String gradient (final String s) {
        ...
        while (matcher.find()) {
            final String str = s.substring(matcher.start() + 9, matcher.end() - 9);
            final String hex1 = s.substring(matcher.start() + 2, matcher.start() + 8);
            final String hex2 = s.substring(matcher.end() - 7, matcher.end() - 1);
